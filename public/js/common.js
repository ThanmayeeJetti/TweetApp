...
$("#pinPostButton").click((event) =>{
    ...
    $.ajax({
        ...
        success: (data,status,xhr) => {
            if(xhr.status!=204){
                alert("couldnot delete the post")
            }
                location.reload();
        }
    })
...
function createPostHtml(postData,largeFont=false) {
    ...
    if(postData.postedBy._id==userLoggedIn._id){
        ...
        buttons=`<button class='pinButton ${pinnedClass}' data-id="${postData._id}" data-toggle="modal" data-target="${dataTarget}"><i class="fa-solid fa-thumbtack"></i></button>
                <button data-id="${postData._id}" data-toggle="modal" data-target="#deletePostModal"><i class="fa-solid fa-times"></i></button>`
    }
...
function messageReceived(newMessage){
    if($(`[data-room="${newMessage}"]`).length==0){
...
function refreshMessagesBadge(){
    $.get("/api/chats",{unreadOnly:true},(data)=>{
        ...
        if(numResults>0){
            ...
            $("#messageBadge").text("").rmeoveClass("active")
        }
    })
}
...
function getNotificationText(notification){
    ...
    if(!userFrom.firstName|| !userFrom.lastName){
        return addListener("user from data not populated")
    }
    ...
    else if(notification.notificationType=="postLiked"){
...
function getNotificationUrl(notification){
    ...
    if(notification.notificationType=="retweet"||
    notification.notificationType=="postLiked"||
